...
class FakeSim(Node):
    ...
    @classmethod
    def _thick_wall(cls, x1, y1, x2, y2, t=0.10):
        """Axis-aligned толстая стена → 4 грани прямоугольника. Концы продлены
        на t/2 чтобы стены аккуратно стыковались в углах перекрёстков."""
        cx, cy = (x1 + x2) / 2, (y1 + y2) / 2
        if x1 == x2:  # вертикальная
            return cls._box(cx, cy, t, abs(y2 - y1) + t)
        else:  # горизонтальная
            return cls._box(cx, cy, abs(x2 - x1) + t, t)
